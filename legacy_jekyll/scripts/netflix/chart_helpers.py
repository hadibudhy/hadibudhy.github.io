...
Usage:
    from helpers.chart_helpers import (
        swd_style, hadi_dark, highlight_bar, highlight_line, action_title,
    ...
    # For hadibudhy.github.io dark theme:
    colors = hadi_dark()   # activates slate/blue palette for Matplotlib + Plotly
"""
...
def load_theme_colors(theme_name: str | None = None) -> dict:
    ...
    COLORS.update({
        ...
        "muted": colors.get("text_muted", colors.get("neutral", "#94a3b8")),
        "bg": colors["background"],
        "surface": colors.get("surface", colors["background"]),
        "text": colors["text"],
        "gray200": colors.get("text_muted", "#94a3b8"),
        "action": colors.get("highlight", {}).get("focus", colors["primary"]),
...
def hadi_dark():
    """Activate the Hadi Dark theme for both Matplotlib and Plotly.

    Tuned for hadibudhy.github.io.
    """
    # 1. Load brand colors into Matplotlib
    theme = load_theme_colors("hadi-dark")

    # 2. Set the special mplstyle if it exists
    style_path = Path(__file__).parent.parent / "themes" / "brands" / "hadi-dark" / "hadi_dark.mplstyle"
    if style_path.exists():
        plt.style.use(str(style_path))
    else:
        swd_style(theme=theme)

    # 3. Apply Plotly template if available
    try:
        from themes.brands.hadi_dark.plotly_template import apply_hadi_dark
        apply_hadi_dark()
    except ImportError:
        pass
...
def swd_style(theme: dict | None = None):
    ...
    if theme is not None:
        ...
        text_muted = colors.get("text_muted", text)
        border = colors.get("border", "#E5E7EB")
        categorical = colors.get("categorical", [])
        ...
        # Apply grid color from theme
        grid_color = colors.get("gridcolor", border)
        plt.rcParams.update({
            "grid.color": grid_color,
        })

        # Apply categorical palette if provided
        if categorical:
            from cycler import cycler
            hex_no_hash = [c.lstrip("#") for c in categorical]
            plt.rcParams["axes.prop_cycle"] = cycler("color", hex_no_hash)
...
def highlight_bar(ax, categories, values, highlight=None, highlight_color=None,
    ...
    if horizontal:
        ...
        # Direct labels
        max_val = max(vals)
        label_color = COLORS.get("text", COLORS["gray900"])
