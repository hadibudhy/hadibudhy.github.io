      ...
      <section className="bg-muted/25">
        <div className="mx-auto max-w-6xl px-4 py-14 sm:px-8 sm:py-20 md:py-24">
          <div className="mb-8">
            ...
            <p className="mt-3 max-w-2xl leading-7 text-muted-foreground">Five studies put the business question, the available evidence, and the limits of each analysis in view.</p>
          </div>
          {projects.length === 0 ? <div className="surface p-8"><p className="text-lg font-semibold text-foreground">No case studies are available yet.</p><p className="mt-2 text-muted-foreground">Please return shortly or use the contact link to request the current portfolio.</p></div> : <div className="grid gap-6 lg:grid-cols-2">{featuredProjects.map((project) => <ProjectCard key={project.slug} slug={project.slug} meta={project.meta} featured />)}</div>}
