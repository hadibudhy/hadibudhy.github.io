...
def save_jolts_signals() -> None:
    ...
    counts = [6.55, 5.203]
    count_bars = axes[0].bar(["Openings", "Total\nseparations"], counts, color=["#5e9bea", "#f36d72"], width=0.58)
    axes[0].set_title("Demand and exits are both large", color="#f3f6fb", fontsize=17, fontweight="bold", pad=20)
    ...
    fig.text(0.5, 0.03, "U.S. BLS JOLTS aggregate | openings and separations are counts; hires and quits are rates",
